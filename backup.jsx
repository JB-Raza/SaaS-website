...
export default function Navbar() {
  ...
  return (
      <header className={`relative z-50 mx-aut`}>
        ...
        <nav ref={navbarRef} className={`rounded-md px-6 ${isFixed ? "fixed shadow-xl top-0 left-0 z-50 right-0 rounded-none" : "absolute left-3 top-5 right-3 max-w-[1350px] mx-auto"} ${!isFixed && location.pathname == "/"? "bg-white/5":""} ${location.pathname == "/" && isFixed?"bg-[#0f6555]":"bg-white"}`}>
